...
export default function App() {
  ...
  const loadHistory = useCallback(async (sid: string) => {
    try {
      const resp = await api(`/sessions/${sid}/messages`);
      if (!resp.ok) return;
  ...
  const activateProvider = useCallback(
    async (name: string) => {
      try {
        const resp = await api(`/providers/${name}/activate`, { method: 'POST' });
  ...
  const handleEvent = useCallback(
    (event: AgentEvent) => {
      ...
      switch (event.type) {
        ...
        case 'abort':
          // AbortEvent（真实后端与 mock 流一致）：中断提示统一由事件驱动
          setIsStreaming(false);
          setStatusText('');
          pushMessage(sid, {
            id: uid(),
  ...
  const switchSession = useCallback(
    (id: string) => {
      ...
      void (async () => {
        try {
          const resp = await api(`/sessions/${id}/mode`);
          if (!resp.ok) return;
  ...
  /** 中断生成：发送 abort，中断提示由 AbortEvent 统一回推展示 */
