...
import type { PermissionModeType } from '../types';
...
/** 底部输入区：Enter 发送 / Shift+Enter 换行 / Esc 中断，自动增高；权限模式内嵌左下角 */
export function ChatInput({ onSend, onAbort, isStreaming, disabled, mode, onModeChange }: ChatInputProps) {
  ...
  return (
    <div className="flex-shrink-0 px-6 pb-4">
      <div className="mx-auto max-w-[760px]">
        <div className="rounded-2xl border border-line bg-elevated px-4 py-3 shadow-card transition-colors focus-within:border-accent/45">
          <textarea
            ref={ref}
            rows={1}
            value={value}
          ...
          <div className="mt-1.5 flex items-center justify-between gap-3">
            ...
            <span className="hidden text-[11px] text-ink-3 sm:block">
              {isStreaming ? 'Esc 中断生成' : 'Enter 发送 · Shift+Enter 换行'}
            </span>
